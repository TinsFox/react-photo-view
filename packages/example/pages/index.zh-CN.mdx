...
<h1 className="text-center font-bold md:text-5xl mt-8">
  @acr/react-photo-view
</h1>
<p className="flex flex-wrap text-center justify-center">
  <img
    alt="stars"
  ...
  <img
    alt="license"
    src="https://badgen.net/npm/license/react-photo-view"
    className="mr-2 mb-2"
  />
  <img
    alt="min"
...
<Callout emoji="🤪">
  <span>@acr/react-photo-view Fork自 👉 </span>
  <a
...
<div className="mt-16 mb-20 text-center">
  [快速上手](/docs/getting-started) · [API](/docs/api) · [GitHub
  仓库](https://github.com/TinsFox/react-photo-view.git) ·
  [原仓库](https://github.com/MinJieLiu/react-photo-view)
</div>
...
该示例中，`@acr/react-photo-view` 提供了两个组件： `PhotoProvider` 和 `PhotoView`。以 `PhotoProvider`为界限，里面所有的 `PhotoView` 图片会按照运行顺序提取为一组图片预览画廊。当某个 `<img />` 被点击，则会定位到指定的图片并打开预览。
