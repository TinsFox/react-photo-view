...
export default function PhotoBox({ item: { src, render, width: customWidth = 0, height: customHeight = 0, originRef }, visible, speed, easing, wrapClassName, className, style, loadingElement, brokenElement, onPhotoTap, onMaskTap, onReachMove, onReachUp, onPhotoResize, isActive, expose, }) {
    ...
    return (React.createElement("div", { className: `PhotoView__PhotoWrap${wrapClassName ? ` ${wrapClassName}` : ''}`, style: style, onMouseDown: !isTouchDevice && isActive ? handleMaskStart : undefined, onTouchStart: isTouchDevice && isActive ? (e) => handleMaskStart(e.touches[0]) : undefined },
        React.createElement("div", { className: "PhotoView__PhotoBox", style: {
            ...
            } }, src ? (React.createElement(Photo, { src: src, loaded: loaded, broken: broken, ...attrs, onPhotoLoad: handlePhotoLoad, loadingElement: loadingElement, brokenElement: brokenElement })) : (render && render({ attrs, scale: currentScale, rotate })))));
}
