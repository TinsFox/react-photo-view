...
export default function PhotoSlider(props) {
    ...
    return (React.createElement(SlidePortal, { className: `PhotoView-Portal${!currentOverlayVisible ? ' PhotoView-Slider__clean' : ''}${!visible ? ' PhotoView-Slider__willClose' : ''}${className ? ` ${className}` : ''}`, role: "dialog", onClick: (e) => e.stopPropagation(), container: portalContainer },
        visible && React.createElement(PreventScroll, null),
        React.createElement("div", { className: `PhotoView-Slider__Backdrop${maskClassName ? ` ${maskClassName}` : ''}${activeAnimation === 1
        ...
        bannerVisible && (React.createElement("div", { className: "PhotoView-Slider__BannerWrap" },
            React.createElement("div", { className: "PhotoView-Slider__Counter" },
                index + 1,
                " / ",
                imageLength),
            React.createElement("div", { className: "PhotoView-Slider__BannerRight" }, toolbarRender && overlayParams ? toolbarRender(overlayParams) : React.createElement(DefaultToolBar, { onClose: onClose })))),
        adjacentImages.map((item, currentIndex) => {
            ...
            return (React.createElement(PhotoBox, { key: enableLoop ? `${item.key}/${item.src}/${nextIndex}` : item.key, item: item, speed: currentSpeed, easing: currentEasing, visible: visible, onReachMove: handleReachMove, onReachUp: handleReachUp, onPhotoTap: () => handlePhotoTap(photoClosable), onMaskTap: () => handlePhotoTap(maskClosable), wrapClassName: photoWrapClassName, className: photoClassName, style: {
                ...
                }, loadingElement: loadingElement, brokenElement: brokenElement, onPhotoResize: handleResize, isActive: (currentImage && currentImage.key) === item.key, expose: updateState }));
        }),
        !isTouchDevice && bannerVisible && (React.createElement(React.Fragment, null,
            (enableLoop || index !== 0) && (React.createElement("div", { className: "PhotoView-Slider__ArrowLeft", onClick: () => changeIndex(index - 1, true) },
                React.createElement(ArrowLeft, null))),
            (enableLoop || index + 1 < imageLength) && (React.createElement("div", { className: "PhotoView-Slider__ArrowRight", onClick: () => changeIndex(index + 1, true) },
                React.createElement(ArrowRight, null))))),
        overlayRender && overlayParams && (React.createElement("div", { className: "PhotoView-Slider__Overlay" }, overlayRender(overlayParams)))));
}
